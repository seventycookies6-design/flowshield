...
| Start time reached, `AskFirst` on, and no card is up for this schedule (the heads-up tick landed while a sprint or break was running, a hand sprint since hid the card, or the clock was moved past the heads-up) | Offer, never start: the same card, reading "Homework evening is due now" with **Start now** and **Skip today** and naming the open apps as the heads-up would, plus the `ScheduledSprint` notification. Nobody was asked, and asking is the one promise Ask first makes. |
